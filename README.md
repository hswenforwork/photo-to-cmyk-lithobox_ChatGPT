...
日式似顏繪風格的AI只會在按下「產生／更新預覽」時執行一次。下載列印頁面、雷射雕刻PNG或ZIP時會直接重用這份已確認的預覽快取，依輸出長寬完整放大後再進行滑桿後製與A4排版，不會再次執行CartoonGAN。若預覽完成後又變更照片、尺寸、裁切、鏡像、遮罩或畫風，工具會要求先更新預覽再下載，避免輸出過期內容。

**下載列印圖層**：所有列印用圖層（原始照片、灰階高度圖、外層裝飾層、C/M/Y三色版）會自動排版到儘量少張的A4紙上——同一張A4能放下幾張圖層就放幾張，並統一預留5mm列印邊界；A4圖面不加入任何文字標籤，各圖層比例與對位記號皆一致，可直接家用印表機列印。雷射雕刻專用灰階PNG維持原始尺寸單獨下載。打包下載ZIP時可勾選是否同時包含另一種風格的檔案。
...
- 日式似顏繪風格為圖像風格轉換（CartoonGAN），效果會依照片內容、光線與構圖而異，並非傳統五官誇張變形的手繪似顏繪；運算成本較高。產生預覽時優先使用相容性較高、不需編譯顯示卡shader的WASM運算；若無法初始化，才依序改用WebGL與CPU備援。WebGL若出現shader、vertex／fragment、program link、貼圖或逾時錯誤，也會自動降級重試。因此即時預覽需手動按「產生／更新預覽」按鈕觸發，不會隨裁切/縮放即時自動更新。所有AI模型下載與預覽運算步驟都有逾時保護（模型下載60秒、單次風格轉換120秒）；下載階段不再執行AI，只會重用已完成的預覽快取。
